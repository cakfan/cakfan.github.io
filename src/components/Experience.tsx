            ...
            <div className="space-y-10">
              {experiences.map((exp, i) => (
                <div key={exp.company + exp.period} className="relative pl-8">
                  <div className={`absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full ${i === 0 ? "border-2 border-teal bg-background dot-pulse" : "border-2 border-teal/50 bg-teal/50"}`} />
                  <div className="text-xs text-muted-foreground mb-1">
                    {exp.period}
                  </div>
                  <h3 className="font-semibold">{exp.role}</h3>
                  <p className="text-sm text-teal mb-2">{exp.company}</p>
